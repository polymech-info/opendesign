...
export function LeftSidebar() {
  const { addText, addShape, addIcon, addImage, addImageFromClipboard, addLibraryElement, setBackground, templates, createFromTemplate, navigate } = useEditor();
              ...
              <div class="flex-1 overflow-y-auto px-3 pb-3">
                {activeSection === "templates" && (
                  <div>
                    {templates.length === 0 ? (
                      ...
                      <>
                        <p class="text-[11px] mb-3 m-0 text-zinc-400">Creates a new design — does not overwrite this one</p>
                ...
                {activeSection === "shapes" && (
                  <div>
                    ...
                    <button
                      class="mt-3 w-full flex items-center justify-center gap-2 p-2.5 rounded-lg bg-white border border-zinc-200 cursor-pointer transition-all hover:border-accent hover:bg-accent/5"
                      onClick={() => void addImageFromClipboard()}
                      title="Paste image from clipboard (Ctrl+V)"
                    >
                      <ClipboardPaste size={16} class="text-zinc-400" />
                      <span class="text-[11px] text-zinc-600">Paste image</span>
                    </button>
                    <p class="text-[10px] text-zinc-400 mt-1.5 m-0">Fits the canvas, then saves to uploads/</p>
                    <p class="text-zinc-400 text-[11px] mb-2 mt-4">Library</p>
                ...
                {activeSection === "icons" && (
                  <div>
                    <p class="text-zinc-400 text-[11px] mb-2">Local: click to place. Iconify: double-click to download to uploads/icons/ and place.</p>
                ...
                {activeSection === "images" && (
                  <div>
                    <p class="text-zinc-400 text-[11px] mb-2">Stored in uploads/ — click or drop images from Explorer to place on the canvas</p>
                ...
                {activeSection === "background" && (
                  <div>
                    ...
                    <p class="text-zinc-400 text-[11px] mb-2">Background images</p>
                    <p class="text-zinc-400 text-[10px] mb-2">Stored in uploads/backgrounds/ — drop an image from Explorer to set it</p>
                    <MediaLibrary kind="backgrounds" onPick={(url) => setBackground("image", url)} />
                  </div>
                )}
